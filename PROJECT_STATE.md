...
- **2026-06-09 — session 12 — 🎨 UX/UI REDESIGN (phone-first messenger) — plan + P0/P1 + P2a/P2b:**
  ...
  - **⏭️ NEXT:** **P4 refinements** — AI/file catalog import (`/api/ai/catalog-import`), per-line catalog autosuggest, true day-timeslot calendar (now an agenda list), appointment duration UI (`tasks.start_at/end_at` migrated but unused). **P2c** Settings 6 categories + Κλήσεις customer-search bar + **SendChannelSheet unify + WhatsApp removal → re-apply migration 042**. **P5** AI assistant (voice / Siri-like popup, customer-scoped — the 🎙️ buttons are still disabled placeholders). **P6** native engine (inbound + server recording + spoken consent + auto missed-call waterfall). The old `customers/[id]` stacked-card page still exists (reached only by direct URL / the info panel is now self-sufficient) — retire in a later pass.
