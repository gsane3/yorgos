...
- **2026-06-09 — session 12 — 🎨 UX/UI REDESIGN (phone-first messenger) — plan + P0/P1 + P2a/P2b:**
  ...
  - **⏭️ NEXT:** **P5** AI assistant (voice / Siri-like popup, customer-scoped — the 🎙️ buttons are still disabled placeholders; needs server STT (Deepgram/OpenAI) + native mic + expanded `/api/ai/cmd` intents incl. send-offer-with-catalog). **P6** native engine (inbound calling-when-closed via Twilio SIP Domain + Push Credential + server recording + spoken GDPR consent + auto missed-call waterfall — external blockers: iOS PushKit, Apifon Viber sender). **P4/P2c refinements** — AI/file catalog import, per-line catalog autosuggest, true day-timeslot calendar (now an agenda list), appointment duration UI, Κλήσεις customer-search bar, retire SendViaViberModal + the old `customers/[id]` stacked-card page (reached only by direct URL now; the Messenger card + info panel are self-sufficient).
